...
def main():
    ...
    print(
        f"PPV = {dict_ppv[(optimal_learning_rate_accuracy, optimal_lambda_accuracy)]}"
    )
    print(
        f"NPV = {dict_npv[(optimal_learning_rate_accuracy, optimal_lambda_accuracy)]}"
    )
    print(
        f"Sensitivity = {dict_sensitivity[(optimal_learning_rate_accuracy, optimal_lambda_accuracy)]}"
    )
    print(
        f"Specificity = {dict_specificity[(optimal_learning_rate_accuracy, optimal_lambda_accuracy)]}"
    )
    print(
        f"F1 score = {dict_F1_score[(optimal_learning_rate_accuracy, optimal_lambda_accuracy)]}"
    )
    ...
    accuracy_score_own = dict_accuracy[
        optimal_learning_rate_accuracy, optimal_lambda_accuracy
    ]
    ...
    print("For this learning rate and lambda we got: ")
    print(
        f"Accuracy = {dict_accuracy[(optimal_learning_rate_ppv, optimal_lambda_ppv)]}"
    )
    print(f"NPV = {dict_npv[(optimal_learning_rate_ppv, optimal_lambda_ppv)]}")
    print(
        f"Sensitivity = {dict_sensitivity[(optimal_learning_rate_ppv, optimal_lambda_ppv)]}"
    )
    print(
        f"Specificity = {dict_specificity[(optimal_learning_rate_ppv, optimal_lambda_ppv)]}"
    )
    print(
        f"F1 score = {dict_F1_score[(optimal_learning_rate_ppv, optimal_lambda_ppv)]}"
    )
    ...
    print("For this learning rate and lambda we got: ")
    print(
        f"Accuracy = {dict_accuracy[(optimal_learning_rate_npv, optimal_lambda_npv)]}"
    )
    print(f"PPV = {dict_ppv[(optimal_learning_rate_npv, optimal_lambda_npv)]}")
    print(
        f"Sensitivity = {dict_sensitivity[(optimal_learning_rate_npv, optimal_lambda_npv)]}"
    )
    print(
        f"Specificity = {dict_specificity[(optimal_learning_rate_npv, optimal_lambda_npv)]}"
    )
    print(
        f"F1 score = {dict_F1_score[(optimal_learning_rate_npv, optimal_lambda_npv)]}"
    )

    print("--------------------------------------------")

    """ plotting heatmap for NPV using own code"""

    ser = pd.Series(
        list(dict_ppv.values()), index=pd.MultiIndex.from_tuples(dict_F1_score.keys())
    )
    df = ser.unstack().fillna(0)
    print(df.shape)
    print(df)
    ...
    print("For this learning rate and lambda we got: ")
    print(
        f"Accuracy = {dict_accuracy[(optimal_learning_rate_sensitivity, optimal_lambda_sensitivity)]}"
    )
    print(
        f"PPV = {dict_ppv[(optimal_learning_rate_sensitivity, optimal_lambda_sensitivity)]}"
    )
    print(
        f"NPV = {dict_npv[(optimal_learning_rate_sensitivity, optimal_lambda_sensitivity)]}"
    )
    print(
        f"Specificity = {dict_specificity[(optimal_learning_rate_sensitivity, optimal_lambda_sensitivity)]}"
    )
    print(
        f"F1 score = {dict_F1_score[(optimal_learning_rate_sensitivity, optimal_lambda_sensitivity)]}"
    )
    ...
    print("For this learning rate and lambda we got: ")
    print(
        f"Accuracy = {dict_accuracy[(optimal_learning_rate_specificity, optimal_lambda_specificity)]}"
    )
    print(
        f"PPV = {dict_ppv[(optimal_learning_rate_specificity, optimal_lambda_specificity)]}"
    )
    print(
        f"NPV = {dict_npv[(optimal_learning_rate_specificity, optimal_lambda_specificity)]}"
    )
    print(
        f"Sensitivity = {dict_sensitivity[(optimal_learning_rate_specificity, optimal_lambda_specificity)]}"
    )
    print(
        f"F1 score = {dict_F1_score[(optimal_learning_rate_specificity, optimal_lambda_specificity)]}"
    )
    ...
    print("For this learning rate and lambda we got: ")
    print(
        f"PPV = {dict_ppv[(optimal_learning_rate_F1_score, optimal_lambda_F1_score)]}"
    )
    print(
        f"NPV = {dict_npv[(optimal_learning_rate_F1_score, optimal_lambda_F1_score)]}"
    )
    print(
        f"Sensitivity = {dict_sensitivity[(optimal_learning_rate_F1_score, optimal_lambda_F1_score)]}"
    )
    print(
        f"Specificity = {dict_specificity[(optimal_learning_rate_F1_score, optimal_lambda_F1_score)]}"
    )
    ...
    print("confusion matrix for optimal F1 score")
    ...
    """ Making a confusion matrix for the  learning rates and lambdas that give optimal Accuracy score"""
    logistic.fit(
        data.X_train,
        data.z_train,
        alpha=optimal_learning_rate_accuracy,
        iterations=10_000,
        lambda_=optimal_lambda_accuracy,
    )
    ...
    print("confusion matrix for optimal accuracy")
    print(confusion_matrix(true_output, prediction_log_reg))
    conf_mat = confusion_matrix(true_output, prediction_log_reg)
    ...
    """ Making a confusion matrix for the  learning rates and lambdas that give highest NPV"""
    logistic.fit(
        data.X_train,
        data.z_train,
        alpha=optimal_learning_rate_npv,
        iterations=10_000,
        lambda_=optimal_lambda_npv,
    )
    ...
    print("confusion matrix for highest NPV")
    print(confusion_matrix(true_output, prediction_log_reg))
    conf_mat = confusion_matrix(true_output, prediction_log_reg)
    ...
    prediction_sklearn_logreg_test = sci_log_reg.predict(data.X_test)

    prediction_sklearn_logreg_test = sci_log_reg.predict(data.X_test) < 0.5
    true_output_sklearn_test = data.z_test < 0.5

    print("--------------------------------------------")

    F1_score_sklearn = f1_score(
        true_output_sklearn_test, prediction_sklearn_logreg_test
    )

    print(f"F1 score using sklearn = {F1_score_sklearn}")
    cnf_matrix_sklearn = metrics.confusion_matrix(
        true_output_sklearn_test, prediction_sklearn_logreg_test
    )
