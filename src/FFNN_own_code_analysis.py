    ...
    for lmb in lambdas:
         for nbr_lay in number_of_hidden_layers:
            for nbr_nodes in nbr_of_hidden_nodes:
                ...
                ffnn_own = FFNN(data.X_train.shape[1],
                                hidden_layers_int,
                                final_layer = SigmoidLayer,
                                classification= True)
                ffnn_own.fit(data.X_train,
                            data.z_train,
                            epochs = 1000,
...
ffnn_own_optimal = FFNN(data.X_train.shape[1],
                        hidden_layers_int_optimal,
                        final_layer = SigmoidLayer,
                        classification= True
                        )

ffnn_own_optimal.fit(data.X_train,
            data.z_train,
            epochs = 1000,
...
print(confusion_matrix(true_output, z_test_FFNN_predict))
conf_mat = confusion_matrix(true_output, z_test_FFNN_predict)
